...
_resourceZones = ["apple_1","apple_2","apple_3","apple_4","peaches_1","peaches_2","peaches_3","peaches_4","heroin_1","cocaine_1","weed_1","rye_1","hops_1","yeast_1","mash_1"];
_zone = "";
...
switch(true) do {
	...
	case (_zone in ["rye_1"]): {_gather = ["Rye",1];};
	case (_zone in ["hops_1"]): {_gather = ["Hops",1];};
	case (_zone in ["yeast_1"]): {_gather = ["yeast",1];};
	case (_zone in ["mash_1"]): {_gather = ["Mash",1];};
	default {""};
};
//gather check??
